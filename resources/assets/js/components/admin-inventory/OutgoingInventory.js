...
export default class OutgoingInventory extends React.Component {
    ...
    render() {
        return (
            <form className="form-group row mt-10">
                ...
                <div className="col-md-12">
                    <button type="button" className="btn btn-primary"  onClick={(e)=> {this.storeOutgoingInventory()}}>
                        { (this.state.is_loading) ? <em className="fa fa-refresh fa-spin" /> : 'Egresar producto(s)'}
                    </button>
                </div>
            </form>
        );
    ...
    storeOutgoingInventory(){
        this.setState({is_loading:true});
        axios.post(this.urloutgoinginventory, {inventory_products:this.state.inventory_products})
            .then((response) => {
...
if (document.getElementsByClassName('admin-outgoing-inventory')) {
    let elements = document.getElementsByClassName('admin-outgoing-inventory');
    let count = elements.length;
    for(let i = 0; i < count; i++) {

        let element = elements[i];
        let data_stores = element.getAttribute("data-stores");
        let urloutgoinginventory = element.getAttribute("url-outgoinginventory");
        let urlstorebranches = element.getAttribute("url-store-branches");


        ReactDOM.render(<OutgoingInventory
