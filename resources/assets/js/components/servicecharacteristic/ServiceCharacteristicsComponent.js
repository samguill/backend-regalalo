...
export default class ServiceCharacteristicsComponent extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.data_service_characteristics = JSON.parse(this.props.data_service_characteristics);
        this.data_service_characteristics_detail = JSON.parse(this.props.data_service_characteristics_detail);
        ...
        this.service_characteristics = this.data_service_characteristics.map((element) => {
            let obj = {};
        ...
        this.state = {
            ...
            service_characteristics: this.data_service_characteristics,
            service_characteristics_detail: this.data_service_characteristics_detail,
            is_loading: false,
            updating: false
        };
    ...
    onSelectCharacteristic(option){
        let characteristic = this.data_service_characteristics.filter((item) => item.id === option.value);
        ...
        this.setState({service_characteristic_id:option.value, values_array:values, values:[]});
    }

    setValues(id){
        let characteristic = this.data_service_characteristics.filter((item) => item.id === parseInt(id));
        let values = characteristic[0].values.map((element) => {
            let obj = {};
            obj["label"] = element.value;
            obj["value"] = element.id;
    ...
    storeCharacteristics(){
        ...
        let data = {
            ...
            service_characteristic_id: this.state.service_characteristic_id,
        ...
        axios.post(url, data)
            .then((response) => {
                ...
                if (this.state.updating){
                    let list = this.state.service_characteristics_detail.map((item)=>{
                        if (response.data.data.id === item.id) {
                            item = response.data.data
                        }
                        return item;
                    });
                    this.setState({service_characteristics_detail:list, is_loading:false})
                }else{
                    this.setState({
                        service_characteristics_detail:this.state.service_characteristics_detail.concat(response.data.data),
                        is_loading:false
                    });
                }
                this.clearForm();
        ...
        swal({
            ...
            axios.post(this.data_delete_url,{id:row}).then((r)=>{
                if(r.data.status ==='ok') {
                    ...
                    this.setState({service_characteristics_detail:this.state.service_characteristics_detail.filter((item) => item.id !== row)});
                }
                    ...
                    <div className="row">
                        <div className="col-md-4 border-right">
                            ...
                            <div className="form-group">
                                ...
                                <Select
                                    ...
                                    name="id"
                                    options={this.service_characteristics}
                                    value={this.state.service_characteristic_id}
                        ...
                        <div className="col-md-8">
                            <table className="table table-responsive">
                                ...
                                <tbody>
                                    {
                                        this.state.service_characteristics_detail.map((row, ri) => {
                                            return <tr key={ri}>
                                                <td>{row.characteristic.name}</td>
                                                <td>{row.service_characteristic_values}</td>
                                                <td>
...
if (document.getElementsByClassName('characteristics-service-component')) {
    let elements = document.getElementsByClassName('characteristics-service-component');
    let count = elements.length;
    for(let i = 0; i < count; i++) {
        let element = elements[i];
        let data_service_characteristics = element.getAttribute("data_service_characteristics");
        let service_id = element.getAttribute("service_id");
        let data_update_url = element.getAttribute("data_update_url");
        let data_store_url = element.getAttribute('data_store_url');
        let data_service_characteristics_detail = element.getAttribute("data_service_characteristics_detail");
        let data_delete_url = element.getAttribute("data_delete_url");

        ReactDOM.render(<ServiceCharacteristicsComponent
            data_service_characteristics={data_service_characteristics}
            data_service_characteristics_detail={data_service_characteristics_detail}
