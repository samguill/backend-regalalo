...
export default class IncomingCoupons extends React.Component {
    ...
    render() {
        return (
            <form className="form-group row mt-10">
                ...
                <div className="col-md-12">
                    <button type="button" className="btn btn-primary"  onClick={(e)=> {this.storeIncomingCoupons()}}>
                        { (this.state.is_loading) ? <em className="fa fa-refresh fa-spin" /> : 'Ingresar Servicio(s)'}
                    </button>
                </div>
            </form>
        );
    ...
    onSelectStore(store){
        ...
        swal({
            title: "Obteniendo sucursales Servicios de la tienda " + store.label,
    ...
    storeIncomingCoupons(){
        ...
        axios.post(this.urlincomingcoupons, {coupons_services:this.state.coupons_services})
            .then((response) => {
...
if (document.getElementsByClassName('admin-incoming-coupons')) {
    let elements = document.getElementsByClassName('admin-incoming-coupons');
    let count = elements.length;
    for(let i = 0; i < count; i++) {

        let element = elements[i];
        let data_stores = element.getAttribute("data-stores");
        let urlincomingcoupons = element.getAttribute("url-incomingcoupons");
        let urlstorebranches = element.getAttribute("url-store-branches");


        ReactDOM.render(<IncomingCoupons
